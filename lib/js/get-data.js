...
function getBloodPressureValueWithoutValues(BPObservations, typeOfPressure) {
  var formattedBPObservations = [];
  BPObservations.forEach(function(observation) {
    var BP = observation.component.find(function(component) {
      return component.code.coding.find(function(coding) {
  ...
  return getQuantityValueWithoutUnit(formattedBPObservations[0]);
}
...
FHIR.oauth2.ready().then(function(client) {
  ...
  client.request("Observation?" + query, {
    ...
    flat: true
      ...
      function(ob) {
        ...
        bloodPressureLevel = getBloodPressureValueWithoutValues(byCodes('55284-4'), '8480-6');
  ...
  function displayHeartRiskDiagnosis(){
    if(typeof sodiumLevel != undefined && typeof bloodPressureLevel != undefined ){
      //console.log("enough data available")
      run(sodiumLevel,bloodPressureLevel);

    }else{
      risk.innerHTML = 'Not enough data on the patient files. so Heart risk diagnosis cannot be made.'
    }
  }
